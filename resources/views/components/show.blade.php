...
<form action="{{ route('notifications.destroy', $notification->id) }}" method="POST" id="deleteNotification_{{ $notification->id }}" class="hidden">
                    @csrf @method('DELETE')
                </form>
                <button type="button" onclick="confirmDelete('deleteNotification_{{ $notification->id }}', 'notifikasi ini')" class="text-sm text-red-400/70 border border-red-400/15 px-4 py-2 rounded-lg hover:text-red-400 hover:bg-red-400/10 transition">
                    Hapus
                </button>
