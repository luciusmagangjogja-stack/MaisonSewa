...
<div class="space-y-6">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div class="flex items-center gap-3">
            <a href="{{ route('users.index') }}" class="btn-secondary p-2">
        ...
        <div class="flex items-center gap-2">
            ...
            @if($user->id !== auth()->id())
            <form method="POST" action="{{ route('users.toggle', $user) }}" id="toggleUserShow_{{ $user->id }}" class="hidden">
                @csrf @method('PATCH')
            </form>
            <button type="button" onclick="confirmAction('toggleUserShow_{{ $user->id }}', '{{ $user->is_active ? 'Nonaktifkan' : 'Aktifkan' }} Pengguna', '{{ $user->is_active ? 'Nonaktifkan' : 'Aktifkan' }} pengguna {{ $user->name }}?', '{{ $user->is_active ? 'Ya, Nonaktifkan' : 'Ya, Aktifkan' }}', '{{ $user->is_active ? '#ef4444' : '#10b981' }}')"
            ...
            <form method="POST" action="{{ route('users.destroy', $user) }}" id="deleteUserShow_{{ $user->id }}" class="hidden">
                  @csrf @method('DELETE')
                </form>
                <button type="button" onclick="confirmDelete('deleteUserShow_{{ $user->id }}', 'pengguna {{ $user->name }}')" class="btn-secondary text-red-500 hover:bg-red-50">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div class="space-y-6">
            ...
            <div class="card p-5 space-y-3">
                ...
                @if($user->id !== auth()->id())
                <form method="POST" action="{{ route('users.toggle', $user) }}" id="toggleUserQuick_{{ $user->id }}" class="hidden">
                    @csrf @method('PATCH')
                </form>
                <button type="button" onclick="confirmAction('toggleUserQuick_{{ $user->id }}', '{{ $user->is_active ? 'Nonaktifkan' : 'Aktifkan' }} Pengguna', '{{ $user->is_active ? 'Nonaktifkan' : 'Aktifkan' }} pengguna {{ $user->name }}?', '{{ $user->is_active ? 'Ya, Nonaktifkan' : 'Ya, Aktifkan' }}', '{{ $user->is_active ? '#ef4444' : '#10b981' }}')"
                ...
                <form method="POST" action="{{ route('users.destroy', $user) }}" id="deleteUserQuick_{{ $user->id }}" class="hidden">
                      @csrf @method('DELETE')
                    </form>
                    <button type="button" onclick="confirmDelete('deleteUserQuick_{{ $user->id }}', 'pengguna {{ $user->name }}')" class="btn-secondary w-full justify-center text-red-500 hover:bg-red-50">
