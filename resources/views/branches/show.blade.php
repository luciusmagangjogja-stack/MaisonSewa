...
<button type="button" onclick="confirmDelete('deleteBranchForm', 'cabang {{ $branch->name }}')" class="btn-secondary text-red-500 hover:bg-red-50">
...
<form id="deleteBranchForm" method="POST" action="{{ route('branches.destroy', $branch) }}" class="hidden">
    @csrf
    @method('DELETE')
</form>
@push('scripts')
