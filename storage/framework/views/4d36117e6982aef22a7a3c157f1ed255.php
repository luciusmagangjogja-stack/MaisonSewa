<?php $__env->startSection('title', 'Detail Penyewaan - ' . $rental->invoice_number); ?>
<?php $__env->startSection('page-title', 'Detail Penyewaan'); ?>
    ...
    >
        <div class="rounded-[20px] border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
            ...
            <div class="mt-5 flex flex-wrap items-center gap-2">
                <a href="<?php echo e(route('rentals.index')); ?>" class="btn-secondary">
                    <i data-lucide="arrow-left" class="w-4 h-4"></i>
                    Kembali ke Penyewaan
                </a>
                <?php if(auth()->user()->isSuperAdmin()): ?>
                <a href="<?php echo e(route('rentals.edit', $rental)); ?>" class="btn-secondary">
                ...
                <a href="<?php echo e(route('rentals.receipt.show', $rental)); ?>" class="inline-flex items-center rounded-xl border border-slate-200 bg-slate-50 px-4 py-2 text-sm font-medium text-slate-700 transition hover:border-blue-300 hover:bg-blue-50 hover:text-blue-700">Receipt</a>
                <a href="<?php echo e(route('rentals.thermal', $rental)); ?>" class="inline-flex items-center rounded-xl border border-slate-200 bg-slate-50 px-4 py-2 text-sm font-medium text-slate-700 transition hover:border-blue-300 hover:bg-blue-50 hover:text-blue-700">Cetak Thermal</a>
                <a href="<?php echo e(route('rentals.receipt.pdf', $rental)); ?>" class="inline-flex items-center rounded-xl border border-slate-200 bg-slate-50 px-4 py-2 text-sm font-medium text-slate-700 transition hover:border-blue-300 hover:bg-blue-50 hover:text-blue-700">Receipt PDF</a>
                <a href="<?php echo e(route('rentals.receipt.whatsapp', $rental)); ?>" class="inline-flex items-center rounded-xl border border-slate-200 bg-slate-50 px-4 py-2 text-sm font-medium text-slate-700 transition hover:border-blue-300 hover:bg-blue-50 hover:text-blue-700">WA Receipt</a>
                <a href="<?php echo e(route('rentals.reminder', $rental)); ?>" class="inline-flex items-center rounded-xl border border-slate-200 bg-slate-50 px-4 py-2 text-sm font-medium text-slate-700 transition hover:border-blue-300 hover:bg-blue-50 hover:text-blue-700">Reminder</a>
                <?php if(auth()->user()->isSuperAdmin()): ?>
                <button type="button" onclick="confirmDeleteRental('<?php echo e($rental->invoice_number); ?>')" class="btn-secondary text-red-500 hover:bg-red-50">
            ...
            <form id="deleteRentalForm" action="<?php echo e(route('rentals.destroy', $rental)); ?>" method="POST" class="hidden">
                <?php echo csrf_field(); ?>
...
<div x-show="guarantee.id_photo_url" class="mt-2">
                ...
                <?php if($rental->rental_status != 'cancelled' && ($rental->payment_status !== 'paid' || ($rental->fine_status !== 'paid' && $rental->fine_status !== 'none'))): ?>
                    <div class="rounded-[20px] border border-slate-200 bg-white p-6 shadow-sm">
                        <div class="mb-4 flex items-center gap-2">
                            <div class="rounded-xl bg-blue-50 p-2 text-blue-600">
                                <svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <form method="POST" action="<?php echo e(route('rentals.payment', $rental)); ?>" class="space-y-3" x-on:submit="preparePaymentSubmit($event)">
                            <?php echo csrf_field(); ?>
                            <div>
                            ...
                            <div>
                                ...
                                <div x-show="paymentMethod === 'qris'" x-transition class="mt-3 rounded-2xl border border-slate-200 bg-white p-4 text-center">
                                    ...
                                    <img src="<?php echo e(asset('images/qris-payment.png')); ?>" alt="QRIS Payment" class="mx-auto h-48 w-48 rounded-xl border border-slate-100 object-contain">
                                </div>
                            </div>
    ...
    <script>
        function returnOperationalDashboard() {
            return {
                ...
                rental: <?php echo json_encode($rentalData, 15, 512) ?>,
                paymentDraft: null,
                paymentMethod: '<?php echo e($rental->payment_method ?? ''); ?>',
                itemCondition: {},
                itemNotes: {},
                itemDamageFees: {},
...
displayedPhotoModal: null,
                ...
                async confirmReturn() {
                    ...
                    try {
                        ...
                        const response = await fetch('<?php echo e(route('rentals.confirm-return-ajax', $rental)); ?>', {
                            method: 'PATCH',
                            headers: {
                ...
                async confirmHandover() {
                    ...
                    try {
                        const response = await fetch('<?php echo e(route('rentals.handover', $rental)); ?>', {
                            method: 'PATCH',
                            headers: {
                                'Accept': 'application/json',
                                'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>',
                ...
                async confirmRefund() {
                    ...
                    try {
                        const response = await fetch('<?php echo e(route('rentals.mark-refund-given', $rental)); ?>', {
                            method: 'POST',
                            headers: {
                                'Accept': 'application/json',
                                'X-CSRF-TOKEN': '<?php echo e(csrf_token()); ?>',
...
<?php echo $__env->make('Layouts.app', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\xampp82\htdocs\rental-jas-main\resources\views/rentals/show.blade.php ENDPATH**/ ?>