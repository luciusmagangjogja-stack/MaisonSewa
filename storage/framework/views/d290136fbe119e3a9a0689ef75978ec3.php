...
<head>
    ...
    <?php echo app('Illuminate\Foundation\Vite')(['resources/css/app.css', 'resources/css/customers.css', 'resources/js/app.js']); ?>
        ...
        <div class="flex h-full flex-col">
            ...
            <div class="mx-4 mt-4 rounded-2xl border border-slate-700 bg-slate-800 p-3">
                <div class="flex items-center gap-3">
                    ...
                    <div class="min-w-0">
                        <div class="truncate text-sm font-bold text-white"><?php echo e($user->name); ?></div>
                        <div class="truncate text-xs font-medium text-blue-300"><?php echo e($user->role_label); ?></div>
                    </div>
                </div>
            </div>
            <?php endif; ?>
            ...
            <nav class="flex-1 overflow-y-auto px-4 py-5">
                <?php $__currentLoopData = $navGroups; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $group => $items): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <?php ($visibleItems = collect($items)->filter(fn($item) => $canSee($item['roles']))); ?>
                    <?php if($visibleItems->isNotEmpty()): ?>
                        <div class="mb-6">
                            <div class="mb-3 px-2 text-[10px] font-bold uppercase tracking-widest text-slate-400"><?php echo e($group); ?></div>
                            <div class="space-y-1">
                                <?php $__currentLoopData = $visibleItems; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    ...
                                    <?php else: ?>
                                        <a
                                            ...
                                            class="sidebar-item flex items-center gap-3 rounded-[16px] px-3 py-2.5 text-sm font-semibold ds-transition <?php echo e(request()->routeIs($item['active']) ? 'active' : 'text-slate-300'); ?>"
                                        >
                                            <i data-lucide="<?php echo e($item['icon']); ?>" class="h-4 w-4"></i>
                                            <span class="truncate"><?php echo e($item['label']); ?></span>
                                        </a>
    ...
    <main class="min-h-screen lg:pl-[260px]">
        ...
        <header class="sticky top-0 z-[80] flex w-full border-b" style="height:72px; background: rgba(255,255,255,.88); border-bottom-color: rgba(226,232,240,.95); backdrop-filter: blur(14px);">
            <div class="h-full w-full flex items-center justify-between px-4 lg:px-8 gap-3">
                ...
                <div class="min-w-0 flex-1 flex items-center gap-3">
                    ...
                    <div class="min-w-0">
                        <div class="hidden sm:flex items-center gap-2 text-[10px] font-bold uppercase tracking-[0.16em] text-slate-500">
                            ...
                            <span class="truncate"><?php echo $__env->yieldContent('page-title', 'Dashboard'); ?></span>
                        </div>
                        <div class="flex items-baseline gap-3">
                            <h1 class="truncate text-xl font-bold tracking-tight text-slate-800 font-sans"><?php echo $__env->yieldContent('page-title', 'Dashboard'); ?></h1>
                            <p class="hidden md:block text-sm font-semibold text-slate-500">
                                Selamat Datang, <span class="text-slate-700"><?php echo e(auth()->user()?->name); ?></span>
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <form action="<?php echo e(route('search.index')); ?>" method="GET" class="hidden lg:flex items-center gap-2 ds-search px-3 ds-transition" style="max-width:560px; width: 560px;">
                    ...
                    >
                    <?php if(request('q')): ?>
                        <button type="button" data-url="<?php echo e(route('search.index')); ?>" onclick="window.location=this.dataset.url" class="ml-2 inline-flex items-center justify-center h-8 w-8 rounded-[14px] hover:bg-blue-50 ds-transition">
                ...
                <div class="flex items-center gap-2">
                    <?php if($overdueCount > 0): ?>
                        <a href="<?php echo e(route('rentals.index', ['status' => 'overdue'])); ?>" class="hidden lg:flex items-center gap-2 rounded-[16px] border border-rose-200 bg-rose-50 px-3 py-2 text-xs font-bold text-rose-700 ds-transition">
                            <i data-lucide="alert-triangle" class="h-4 w-4"></i>
                            <?php echo e($overdueCount); ?> telat
                        </a>
                    <?php endif; ?>

                    <button type="button" class="lg:hidden rounded-[16px] border border-slate-200 bg-white p-2 ds-transition" @click="window.location='<?php echo e(route('search.index')); ?>'">
                        <i data-lucide="search" class="h-5 w-5 text-slate-700"></i>
                    </button>

                    <a href="<?php echo e(route('notifications.index')); ?>" class="relative rounded-[16px] border border-slate-200 bg-white p-2 shadow-sm hover:shadow-md ds-transition">
                        ...
                        <?php if($unreadNotif > 0): ?>
                            <span class="absolute -right-1 -top-1 flex h-5 min-w-5 items-center justify-center rounded-full bg-blue-600 px-1 text-[10px] font-bold text-white">
                                <?php echo e($unreadNotif > 9 ? '9+' : $unreadNotif); ?>
                    ...
                    <div class="relative" x-data="{ open:false }">
                        ...
                        <div x-show="open" @click.outside="open=false" x-transition
                            ...
                            <div class="px-2 py-2">
                                ...
                                <div class="text-xs font-medium text-slate-500"><?php echo e(auth()->user()->email); ?></div>
                            </div>
                            <div class="border-t my-2 border-slate-200"></div>
                            <a href="<?php echo e(route('profile.edit')); ?>" class="flex items-center gap-2 rounded-[14px] px-2 py-2 text-sm font-semibold text-slate-700 hover:bg-blue-50 ds-transition">
                                <i data-lucide="user" class="h-4 w-4"></i>
                                Profil Saya
                            </a>
<?php if(auth()->guard()->check()): ?>
                            <?php if(auth()->user()->isSuperAdmin()): ?>
                                <a href="<?php echo e(route('settings.index')); ?>" class="mt-1 flex items-center gap-2 rounded-[14px] px-2 py-2 text-sm font-semibold text-slate-700 hover:bg-blue-50 ds-transition">
        ...
        <section class="px-4 py-6 lg:px-8 pb-28 lg:pb-6">
            <div class="space-y-6">
                <?php if ($__env->exists('components.flash-messages')) echo $__env->make('components.flash-messages', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
    ...
    <div class="fixed inset-x-0 bottom-0 z-40 lg:hidden" style="padding-bottom: max(.5rem, env(safe-area-inset-bottom));">
        <div class="mx-3 mb-3 rounded-[20px] border border-slate-200 bg-white/88 backdrop-blur-xl shadow-[0_14px_50px_rgba(15,23,42,0.10)]">
            <div class="grid grid-cols-4 gap-1 p-2">
                <a href="<?php echo e(route('dashboard')); ?>" class="ds-hover-lift flex flex-col items-center justify-center gap-1 rounded-[16px] py-2 <?php echo e(request()->routeIs('dashboard') ? 'bg-blue-50 text-blue-600 border border-blue-200' : 'bg-transparent text-slate-500 hover:bg-blue-50 border border-transparent'); ?>" style="border-color: rgba(226,232,240,.95);">
                ...
                <a href="<?php echo e(route('customers.index')); ?>" class="ds-hover-lift flex flex-col items-center justify-center gap-1 rounded-[16px] py-2 <?php echo e(request()->routeIs('customers.*') ? 'bg-blue-50 text-blue-600 border border-blue-200' : 'bg-transparent text-slate-500 hover:bg-blue-50 border border-transparent'); ?>" style="border-color: rgba(226,232,240,.95);">
                    <i data-lucide="users" class="h-5 w-5"></i>
                    <span class="text-[10px] font-bold">Customers</span>
                </a>
                <a href="<?php echo e(route('notifications.index')); ?>" class="ds-hover-lift flex flex-col items-center justify-center gap-1 rounded-[16px] py-2 bg-transparent text-slate-500 hover:bg-blue-50 border border-slate-200 relative">
...
<?php /**PATH C:\xampp82\htdocs\rental-jas-main\resources\views/Layouts/app.blade.php ENDPATH**/ ?>